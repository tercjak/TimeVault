...
class Vault:
    def __init__(self, data_dir: str = ""):
        ...
        self.storage = Storage.__init__(data_dir=data_dir)
        self.apis = {}
    ...
    async def get_data(self, source: str, symbols: List[str], start_time: datetime, end_time: datetime):
        ...
        async with trio.open_nursery() as nursery:
            for symbol in symbols:
                ...
                if shouldDownload:
                    nursery.start_soon(self._download_and_process, downloader, processor, source, symbol, newStartTime,
                                       newEndTime, symbols)

    async def _download_and_process(self, downloader: Downloader, processor: DataProcessor, source: str,
                                    symbol: str, interval, start_time: datetime, end_time: datetime, symbols):
        with tqdm(total=None, desc=f"Downloading {symbol} from {source}", unit="B", unit_scale=True,
                  unit_divisor=1024) as progress_bar:
            try:
                df = await downloader.download(symbol, interval, start_time, end_time, progress_bar)
